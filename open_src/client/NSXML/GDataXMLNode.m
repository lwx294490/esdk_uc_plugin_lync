...
+ (GDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {
	return [GDataXMLNode elementWithName:name stringValue:value type:NO];
}

+ (GDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value type:(BOOL)needCDATA{
	xmlNodePtr theNewNode = xmlNewNode(NULL, // namespace
									   GDataGetXMLString(name));
	if (theNewNode) {
		...
		if (textNode) {
			...
			if (temp) {
				if (needCDATA) {
					//theNewNode->type = XML_CDATA_SECTION_NODE;
				}
				// succeeded
				return [self nodeConsumingXMLNode:theNewNode];
			}

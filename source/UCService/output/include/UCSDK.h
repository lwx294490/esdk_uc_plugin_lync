...
//前传业务类型参数
enum ForwardServiceType
{
	FORWARD_UNCONDITION = 0,	//无条件前传
	FORWARD_ONBUSY,				//遇忙前传
	FORWARD_NOREPLY,			//无应答前传
	FORWARD_OFFLINE,			//离线前传
	VOICEMAIL_UNCONDITION,		//无条件转语音邮箱
	VOICEMAIL_ONBUSY,			//遇忙转语音邮箱
	VOICEMAIL_NOREPLY,			//无应答转语音邮箱
	VOICEMAIL_OFFLINE,			//离线转语音邮箱
	FORWARD_BORDER				//边界
};
...
typedef enum  
{
	TRANSFER_TYPE	=	0,		//转移
	CALLPARK_TYPE	=	1,		//驻留
	CALLSWITCH_TYPE	=	2,		//一键转接
	CALLTRACE_TYPE	=	3,		//恶意跟踪
	DEFAULT_TYPE,				//默认
} CALLTRANS_TYPE;
...
typedef struct
{
	...
	int isConference;           //1表示conference ，0表示是点对点呼叫
} STAudioVideoParam;
...
typedef struct
{
	char Num[STRING_LENGTH];	//号码
}STCallExData; 

typedef enum
{
	File_SendAckNotify = 0,
	File_AcceptedNotify,
	File_ReceivedNotify,
	File_CancelNotfiy,
...
typedef struct
{
	char convid[STRING_LENGTH];		/*!< 会话id*/
	char transferid[STRING_LENGTH]; /*!< 文件传输id*/
	char sender[STRING_LENGTH];		/*!< sender's url*/
	char sendername[STRING_LENGTH]; /*!< sender's name*/
	char filename[STRING_LENGTH];	/*!< 文件全路径*/
	char time[STRING_LENGTH];		/*!< 文件全路径*/
	char reason[STRING_LENGTH];		/*!< cancel or reject reason*/
	char errcode[STRING_LENGTH];	/*!< cancel or reject errorcode*/
	char filesize[STRING_LENGTH];	/*!< 文件大小*/
	char cursize[STRING_LENGTH];	/*!< 已接收或已发送大小*/
	char procent[STRING_LENGTH];	/*!< 文件传输*/
	File_TransNotifyType notifyType; /*< type of received notify */
}STFileTransParam;

typedef enum{
	EM_BLIND_TRANS,		//盲转
	EM_CONSLUT_TRANS,	//咨询转
}TRANS_TYPE;	//本地转移类型
...
{
	int membertype;					//成员类型
	char account[STRING_LENGTH];	//成员账户
}STConfMember;
//新增会议列表
typedef struct
{
	int size;					//新增与会者个数
	STConfMember stConfMem[1];	//与会者列表
}STConfMemList;
...
typedef void (CALLBACK *CallTransEventCB)(int type,int reslut);	//呼叫转移结果通知事件
#endif
